...
try:
    ...
    try:
        ...
        pass
    # Clean Image URL: replace missing/unknown with placeholder
    try:
        def _clean_img(val):
            if pd.isna(val):
                return 'https://via.placeholder.com/400x400?text=Jewelry+Image'
            s = str(val).strip()
...
async def get_recommendations(response: SurveyResponse):
    ...
    filtered = JEWELRY_DF[
        (JEWELRY_DF['Style'].str.lower() == response.style.lower()) &
        (JEWELRY_DF['Price'] <= response.budget)
    ]
    
    if filtered.empty:
        # If no exact style match, try to find similar styles or just get random items within budget
        filtered = JEWELRY_DF[JEWELRY_DF['Price'] <= response.budget]
    
    # Convert to list of dictionaries and shuffle
    recommendations = filtered.sample(min(3, len(filtered))).to_dict('records')
